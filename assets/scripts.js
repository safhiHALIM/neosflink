...
// Fonction pour générer automatiquement un nouveau lien pour un produit
function generateNewLinkForProduct(productId, reason = 'auto') {
  ...
  const newLinkData = { 
    ...
    autoGenerated: true,
    generationReason: reason // 'auto', 'purchase', 'expiry'
  };
  localStorage.setItem('productlink_' + newUuid, JSON.stringify(newLinkData));
  
  console.log(`🔗 Nouveau lien généré pour le produit ${productId} (${reason}):`);
...
// Fonction pour maintenir un lien actif en permanence pour chaque produit
function maintainActiveLinkForProduct(productId) {
  const oneHour = 3600000; // 1 heure en millisecondes
  
  // Vérifier s'il y a un lien actif pour ce produit
  const keys = Object.keys(localStorage);
  ...
  if (!hasActiveLinkForProduct) {
    generateNewLinkForProduct(productId, 'maintenance');
  }
  
  // Programmer la vérification suivante dans 1 heure
  setTimeout(() => {
    maintainActiveLinkForProduct(productId);
  }, oneHour + 1000);
}

// Fonction pour démarrer la maintenance automatique pour tous les produits
function startAutoLinkMaintenance() {
  products.forEach(product => {
    // Démarrer immédiatement la maintenance pour chaque produit
    maintainActiveLinkForProduct(product.id);
    
    // Programmer une régénération toutes les heures (même si lien actif)
    setInterval(() => {
      // Expirer le lien actuel et générer un nouveau
      const keys = Object.keys(localStorage);
      keys.forEach(key => {
        if (key.startsWith('productlink_')) {
          try {
            const linkData = JSON.parse(localStorage.getItem(key));
            if (linkData && linkData.productId === product.id && !linkData.expired && !linkData.purchaseCompleted) {
              // Expirer l'ancien lien
      ...
      generateNewLinkForProduct(product.id, 'hourly_rotation');
    }, oneHour); // Exactement toutes les heures
  });
}

// Fonction legacy pour compatibilité
function autoGenerateNewLinkAfterExpiry(productId) {
  return generateNewLinkForProduct(productId, 'legacy');
}
...
function showActiveLinks() {
  ...
  console.log('🔗 === RAPPORT DES LIENS NEOLINK3 ===');
  ...
  if (activeLinks.length > 0) {
    ...
    activeLinks.forEach(link => {
      console.log(`   🔗 ${link.productId} (${link.uuid})`);
      console.log(`      ⏰ Temps restant: ${link.timeRemainingFormatted}`);
      console.log(`      🔧 Généré: ${link.generationReason} | Auto: ${link.autoGenerated}`);
      console.log(`      🌐 URL: ${link.url}`);
      console.log('');
    });
  }
  
  const expiredLinks = allLinks.filter(link => !link.isActive);
  if (expiredLinks.length > 0) {
    console.log('❌ LIENS EXPIRÉS/UTILISÉS:');
    expiredLinks.forEach(link => {
      console.log(`   🔗 ${link.productId} (${link.uuid}) - ${link.status}`);
      console.log(`      📅 Créé: ${link.createdAt}`);
      if (link.expiredReason) {
        console.log(`      💀 Raison expiration: ${link.expiredReason}`);
      }
      console.log('');
    });
  }
  
  return { allLinks, activeLinks, expiredLinks };
}
...
if (window.location.pathname.endsWith('product.html')) {
  ...
  window.markLinkUsed = function(uuid) {
    ...
    if (linkData) {
      ...
      // Générer immédiatement un nouveau lien pour ce produit après achat
      setTimeout(() => {
        const newUuid = generateNewLinkForProduct(linkData.productId, 'purchase');
        console.log(`💳 Achat validé ! Nouveau lien généré automatiquement pour ${linkData.productId}`);
        console.log(`🔗 Nouveau lien disponible: product.html?id=${linkData.productId}&uuid=${newUuid}`);
...
window.generateNewLinkForProduct = generateNewLinkForProduct;
window.maintainActiveLinkForProduct = maintainActiveLinkForProduct;
window.startAutoLinkMaintenance = startAutoLinkMaintenance;

// --- Démarrage automatique du système de maintenance des liens ---
// Démarrer le système de maintenance automatique des liens pour tous les produits
startAutoLinkMaintenance();
